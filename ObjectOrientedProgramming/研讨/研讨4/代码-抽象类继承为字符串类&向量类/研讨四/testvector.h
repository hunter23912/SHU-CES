...
void testvector1() {
	...
	cout << "提供复制构造函数:myvector<int> d(a)得到的d为：" << d << endl;
...
void testvector3() {
	...
	// 测试resize函数
	vec.resize(10);
	cout << "调用resize(10)后，修改size和capacity后，myvector 大小为：" << vec.size() << " 容量为：" << vec.capacity() << endl;

	// 测试reserve函数
	vec.reserve(17);
	cout << "调用reserve(17)后，只修改capacity,不修改size后,myvector 大小为：" << vec.size() << " 容量为：" << vec.capacity() << endl;
